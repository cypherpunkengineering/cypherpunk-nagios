...
class Client:
    def __init__(self, settings, callback, userpass=None):
        ...
        self.proto = self.settings.get('proto')
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.init_state == State.CONTROL_CHANNEL_OPEN:
                ...
                self.callback(self)
            elif self.init_state == State.PULL_REQUEST_SENT:
                self.ctrl.send_push_request()
                time.sleep(0.1)
            ...
            self.handle_in()
            self.ctrl.handle_in()
            if self.init_state == State.INITIALIZED:
                self.data.handle_in()
    ...
    def on_push(self, data):
        ...
        #assert self.init_state == State.KEY_EXCHANGED
    ...
    def handle_in(self):
        try:
            if self.proto == "tcp":
                packet_size = int.from_bytes(self.socket.recv(2), 'big')
    ...
    def _send(self, data):
        if self.proto == "tcp":
            packet_size = (len(data)).to_bytes(2, 'big')
            data = packet_size + data
    ...
    def _recv(self):
        if self.proto == "tcp":
            ...
            #return self.socket.recv(8192)[2:]
