...
class DataChannel(Channel):
    ...
    def send(self, payload):
        ...
        #plaintext += b'\x66'  # no compression
        plaintext += payload
        #plaintext += lzo.compress(payload, 15)
    ...
    def handle_in(self):
        ...
        if self.queue:
            ...
            #self.log.debug("compression byte: %r", compression)
            # http://build.openvpn.net/doxygen/html/comp_8h_source.html
            #assert compression == 0xfa  # no compression
            #assert compression == 0x66  # no compression

            #payload = plaintext[5:]
            payload = plaintext[4:]
            #payload = lzo.decompress(payload)
